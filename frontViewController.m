...
//  DAScroll
//
//  Created by Anthony Jonikas on 2/11/16.
...
-(id) initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil{
    ...
    if (self) {
        for (int i = 1; i < 2; i++){//cycles through 5 pages of the deerfieldscroll API
            ...
            for (int j=0; j<jsonDataArray.count; j++){
                ...
                if ([sectionName isEqualToString:@"Front"]){
                    //accessing the title
...
{
    for (int i = _loadedArticles; i < _loadedArticles+2; i++){//cycles through 5 pages of the deerfieldscroll API
        //this chunk of text is the URLRequest
        NSMutableURLRequest *request = [[NSMutableURLRequest alloc] init];
        _size = 30;
        NSString *websiteURL = [NSString stringWithFormat:@"http://www.deerfieldscroll.com/wp-json/posts/?page=%i", i];
...
-(IBAction) changeTheFontSize2:(id) sender
{
    //need to make it so the first time all it does is make the size 29
    _textView.font = [UIFont fontWithName:@"Helvetica Light" size:_size-1];
    _size = _size -1;
}
